...
{% hint style="info" %}
Before importing, export existing data to confirm the correct Key is used to avoid creating duplicate contacts.
{% endhint %}
...
{% hint style="info" %}
If your brand offers a membership program or loyalty rewards, we recommend binding contacts via `customer_id` to enable personalization and advanced automation.
{% endhint %}
...
<summary>Which field should I use as the Key?</summary>

Choose the Key based on your data source:
...
<summary>If I use phone as the Key, which contact will be updated?</summary>

The system will update the **most recently created** contact with the same phone number.
...
<summary>Can I use `external_member_id` for import or update?</summary>

No. This field is export-only and cannot be used for import or update operations.
